...
#pragma mark -------------- 随机生成一个小于32位随机数 --------------
//随机生成一个小于32位随机数
+(NSString *)generateStochasticNumber{
    NSArray * array = @[@"a",@"b",@"c",@"d",@"e",@"f",@"g",@"h",@"i",@"j",@"k",@"l",@"m",@"n",@"o",@"p",@"q",@"r",@"s",@"t",@"u",@"v",@"w",@"x",@"y",@"z",@"1",@"2",@"3",@"4",@"5",@"6",@"7",@"8",@"9",@"0"];
    NSString * str = nil;
    for (int  i = 0; i < 32; i++) {
        if (i == 0) {
            str = [NSString stringWithFormat:@"%@",array[arc4random()%32]];
        }else{
            str = [NSString stringWithFormat:@"%@%@",str,array[arc4random()%32]];
        }
    }
    return str;
}

//加边框后 label左右两边空白距离相等
+(UILabel *)getAttributedTextWithSrting:(NSString *)text AndLabel:(UILabel *)label;
{
    ...
    [UIView setViewCorner:label cornerRadius:2 borderWidth:1 colorRGB:(0xff4d64)];
