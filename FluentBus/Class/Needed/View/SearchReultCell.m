...
- (void)prepareUI {
    ...
    self.leftImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"searchbar_textfield_search_icon"]];
    self.centerLabel = [[UILabel alloc] init];
    self.centerLabel.textColor = UIColorWithRGB(51, 51, 51, 1);
    self.centerLabel.font = [UIFont systemFontOfSize:14];
    self.rightLabel = [[UILabel alloc] init];
    self.rightLabel.textColor = UIColorWithRGB(153, 153, 153, 1);
    self.rightLabel.font = [UIFont systemFontOfSize:12];
    self.bottomLine = [[UIView alloc] init];
    self.bottomLine.backgroundColor = UIColorWithRGB(221, 221, 221, 0.5);
    ...
    [self.centerLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.leading.equalTo(self.leftImageView.mas_trailing).offset(5);
        make.centerY.equalTo(self);
    }];
