//
//  FBChangePhoneFirstViewController.m
//  FluentBus
//
...
@interface FBChangePhoneFirstViewController ()
{
    NSInteger _second;
    NSTimer *_timer;
}
...
@implementation FBChangePhoneFirstViewController
...
- (void)viewDidLoad {
    ...
    self.phoneNumberTF.text = [UserInfo userInfoWithUserDefaults].userName;
...
- (IBAction)onClickNextBtn:(id)sender {
    ...
    [OCNetworkingTool POSTWithUrl:checkVCode() withParams:params success:^(id responseObject) {
        DBLog(@"%@",responseObject);
        
        if ([responseObject[@"code"] integerValue] ==200) {
            [MBProgressHUD showSuccess:@"验证成功"];
            [self.navigationController pushViewController:[FBChangePhoneSecondViewController new] animated:YES];
        }else{
