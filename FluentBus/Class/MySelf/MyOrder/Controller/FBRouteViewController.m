...
@interface FBRouteViewController ()<AMapNaviDriveManagerDelegate, AMapNaviDriveViewDelegate,FBMapMidTableViewDelegate,JF_CalendarViewDelegate,UIActionSheetDelegate,MAMapViewDelegate,AMapSearchDelegate>{
...
-(void)getMapStationArrayData{
    ...
    if ([_viewStyle integerValue] == 1000) {
        ...
        self.startCoordinate        = CLLocationCoordinate2DMake(self.startPoint.latitude, self.startPoint.longitude);
        self.destinationCoordinate  = CLLocationCoordinate2DMake(self.endPoint.latitude, self.endPoint.longitude);
...
#pragma mark - 创建地图
-(void)creatMap{
#pragma mark - 这里添加一个判断，如果是需要放大地图的功能（订票界面（2）进来的当点击单个站点的时候需要放大地图），这里就创建普通地图，否则就创建导航地图，导航地图划线比较圆滑
    if ([_viewStyle integerValue] == 1000) {
        //普通地图
...
- (void)driveManager:(AMapNaviDriveManager *)driveManager showCrossImage:(UIImage *)crossImage{
    if (driveManager == self.driveManager) {
        crossImage = UIImageName(@"arrowTexture");
    }
}
#pragma mark - 懒加载
- (UIView *)blackView2{
...
- (void)addDefaultAnnotations{
    ...
    [_maMapView addAnnotation:startAnnotation];
    [_maMapView addAnnotation:destinationAnnotation];
}
//添加途经点
...
- (void)presentCurrentCourse
{
    ...
    [self.naviRoute addToMapView:_maMapView];
    
    /* 缩放地图使其适应polylines的展示. */
    [_maMapView setVisibleMapRect:[CommonUtility mapRectForOverlays:self.naviRoute.routePolylines]
                        edgePadding:UIEdgeInsetsMake(RoutePlanningPaddingEdge, RoutePlanningPaddingEdge, RoutePlanningPaddingEdge, RoutePlanningPaddingEdge)
                           animated:YES];
}

#pragma mark -  ---------------- 这里测试普通地图规划路线的方法
