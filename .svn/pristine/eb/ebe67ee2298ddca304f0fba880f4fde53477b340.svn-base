...
@interface FBMyOrderDetailsViewController ()<JF_CalendarViewDelegate>{
    ...
    FBRefundByOrderModel * refundByOrderModel;
...
-(void)dataHttps{
    ...
    [FBMyOrderDetailsModel userOrderDetailsOrderID:_orderID Success:^(id successed) {
        ...
        if ([_myOrderModel.orderStatus integerValue] == 4 || [_myOrderModel.orderStatus integerValue] == 5 || [_myOrderModel.orderStatus integerValue] == 6) {
            //请求退款的信息
            [FBRefundByOrderModel getRefundByOrderId:_orderID Success:^(id successed) {
                refundByOrderModel = (FBRefundByOrderModel *)successed;
...
-(void)creatView{
    ...
    _tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 64, SCREEN_WIDTH, SCREEN_HEIGHT - 80) style:UITableViewStylePlain];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0) {
        ...
        float h = [PublicMethod handelHighContent:refundByOrderModel.refuseReason AndFontsize:FONT(13) AndMaxsize:CGSizeMake(SCREEN_WIDTH - 32, 1000)];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 2) {
        if ([detailsModel.orderStatus integerValue] == 4) {
            ...
            threeView.ConTextLabel.text = refundByOrderModel.refuseReason;
            threeView.titleLabel.text = @"退款信息";
            float h = [PublicMethod handelHighContent:refundByOrderModel.refuseReason AndFontsize:FONT(13) AndMaxsize:CGSizeMake(SCREEN_WIDTH - 16, 1000)];
            [threeView mas_makeConstraints:^(MASConstraintMaker *make) {
                make.top.equalTo(cell.contentView);
                make.leading.equalTo(cell.contentView);
                make.width.mas_equalTo(SCREEN_WIDTH);
                //高度动态及计算
                make.height.mas_equalTo(50 + h);
            }];
        }else if ([detailsModel.orderStatus integerValue] == 5){
            ...
            [refundSuccessView setRefundByOrderModel:refundByOrderModel];
        }else{
            //如果要显示退款失败的票的信息，则显示下面的一部分
//            refundSuccessView = [[[NSBundle mainBundle]loadNibNamed:@"FBMyOrderDetailsRefundSuccessView" owner:self options:0]lastObject];
//            [cell.contentView addSubview:refundSuccessView];
//            refundSuccessView.translatesAutoresizingMaskIntoConstraints = NO;
//            [refundSuccessView mas_makeConstraints:^(MASConstraintMaker *make) {
...
//            [refundSuccessView setRefundByOrderModel:refundByOrderModel];
            ...
            [cell.contentView addSubview:threeView];
            threeView.ConTextLabel.text = refundByOrderModel.refuseReason;
            threeView.titleLabel.text = @"拒绝原因";
            float h = [PublicMethod handelHighContent: refundByOrderModel.refuseReason AndFontsize:FONT(13) AndMaxsize:CGSizeMake(SCREEN_WIDTH - 32, 1000)];
            [threeView mas_makeConstraints:^(MASConstraintMaker *make) {
                make.top.equalTo(cell.contentView);
                make.leading.equalTo(cell.contentView);
