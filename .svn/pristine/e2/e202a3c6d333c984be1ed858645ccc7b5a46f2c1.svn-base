...
- (void)addSearchResultListView {
    ...
    self.pullDownTableView.selectedPullDownCellCallback = ^(NSIndexPath *indexPath,id selectedPlace){
        ...
        weakSelf.placeModel.addressName = tip.address;
        // 添加大头针
        CLLocationCoordinate2D coor = CLLocationCoordinate2DMake(tip.location.latitude, tip.location.longitude);
        [weakSelf getNearData:coor];
...
-(void)mapView:(MAMapView *)mapView regionDidChangeAnimated:(BOOL)animated{
    ...
    if (isTouch == YES) {
        ...
        [self getNearData:_centerAnnotation.coordinate];
    }
}
#pragma mark - 获取附近的数据
-(void)getNearData:(CLLocationCoordinate2D)coord{
    AMapPOIAroundSearchRequest * reauest = [[AMapPOIAroundSearchRequest alloc]init];
    reauest.location = [AMapGeoPoint locationWithLatitude:coord.latitude longitude:coord.longitude];
...
- (void)popConfirmButton{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        weakSelf.confirmButton.frame = CGRectMake(weakSelf.originalFrame.origin.x, SCREEN_HEIGHT - View_Height(self.nearMapView) - 10 - weakSelf.originalFrame.size.height, weakSelf.originalFrame.size.width, weakSelf.originalFrame.size.height);
    }];
}
...
//        annotationView.canShowCallout =YES;
         annotationView.image = [UIImage imageNamed:@"45v44184178914"];
        return annotationView;
    }
    return nil;
}
    ...
    if ([request isKindOfClass:[AMapPOIAroundSearchRequest class]]) {
     ...
     //下面view上的数据
        ...
        for (AMapPOI *p in response.pois) {
            DBLog(@"%@-------%@",self.placeModel.name,self.placeModel.addressName);
...
- (void)cellDidClickCallbackWith:(AMapPOI *)place {
    ...
    self.placeModel.addressName = place.address;
      // 添加大头针
...
-(FBMapAllroundView *)nearMapView{
    if (!_nearMapView) {
        _nearMapView = [[FBMapAllroundView alloc]initWithFrame:CGRectMake(0, SCREEN_HEIGHT * 2 / 3, SCREEN_WIDTH, SCREEN_HEIGHT / 3)];
